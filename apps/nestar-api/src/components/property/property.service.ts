...
export class PropertyService {
	...
	public async createProperty(input: PropertyInput): Promise<Property> {
		try {
			...
			console.log('Error, propertyModel', err.massage);
	...
	public async updateProperty(
		...
		let { propertyStatus, soldAt, deletedAt } = input; //distraction
	...
	public async getProperties(
		...
		const sort: T = {
			[input?.sort ?? 'creataedAt']: input?.direction ?? Direction.DESC,
		};
				...
				{ $sort: sort },
				{
					$facet: {
						list: [
							...
							lookupMember, //[Property1memberID, property2memberid, property3+memberdata]
							{ $unwind: '$memberData' },
						],
	...
	public async likeTargetProperty(
		...
		const input: LikeInput = {
			memberId: memberId,
			likeRefId: likeRefId,
			likeGroup: LikeGroup.PROPERTY, //enum qiymatni tashkil qilishi kerak
		};
			...
			.aggregate([
				...
				{ $sort: sort },
				{
					$facet: {
						list: [
							{ $skip: (input.page - 1) * input.limit },
							{ $limit: input.limit }, //[propery1,property2]
							lookupMember, //memberData:[memberDataValue]
							{ $unwind: '$memberData' }, //membrData: memberDataValue
						],
						metaCounter: [{ $count: 'total' }],
					},
				},
